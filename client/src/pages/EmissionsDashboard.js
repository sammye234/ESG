...
const BreakdownTable = ({ data = [], title, color, totalEmissions }) => {
  
  const scopeTotal = data.reduce((sum, item) => sum + (item.value || 0), 0);


  const referenceTotal = totalEmissions || scopeTotal;
              ...
              data.map((item, idx) => {
                ...
                return (
                  <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-2 font-medium text-gray-800">{item.name || 'Unknown'}</td>
                    <td className="py-3 px-2 text-right text-gray-900">{value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
